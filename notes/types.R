...
list_y = list('Chicago' = 1, 'New York' = 2, 'Los Angeles' = 3)
...
# which never change the data-types from the input
# Never change the names of variables, never change the Row names
# mamke printing easier
# Overall, you can make more changes to data frames, but tibbles are 
# easier to use. The tibble package is part of the core tidyverse. 
# So, if you’ve already installed the tidyverse, you have what you 
# need to start working with tibbles. 
as_tibble(diamonds)


# The main difference between element-wise logical operators (&, |) 
# and logical operators (&&, ||) is the way they apply to operations with vectors. 
# The operations with double signs, AND (&&) and logical OR (||), only examine 
# the first element of each vector. The operations with single signs, AND (&) 
# and OR (|), examine all the elements of each vector. 
vec_a = c(0, 2, 3, 4, 6, 8, 10)
vec_b = c(0, 2, 3, 4, 5, 6, 7)

# therefore, this should return true
print(vec_a & vec_b) 
# while this should return false as it will only evaluate the first elements 
# of these vectors
print(vec_a[1] && vec_b[1])
